...
```{r}
### FINISH CODE HERE
p <- ggplot(cv_states_today_filter, aes(x=pop_density, y=deathsper100k, size=population)) + geom_point() + geom_smooth(method = "loess", se = TRUE)
ggplotly(p)
```
The LOESS smoothing curve shows that initially, as `pop_density` increases, the `newdeathsper100k` appears to decrease slightly, but it then stabilizes and gradually increases as population density continues to rise. I think `pop_density` is not a correlate of `newdeathsper100k`.
...
```
In October 2021, Alaska had the highest case fatality rate (CFR), followed by Montana, Idaho, Wyoming and North Dakota, indicating that a significantly higher proportion of COVID-19 cases in these areas resulted in death than in other areas. In November 2024, all states had low CFRs, reflecting significant progress in controlling COVID-19.
